...
set NETS=%LOGS%\NetS9_%TS%.log
set NETC=%LOGS%\NetC9_%TS%.log

taskkill /F /IM UnrealEditor.exe >nul 2>&1
ping 127.0.0.1 -n 3 >nul

echo Starting dedicated server (log: %NETS%)...
start "NetServer9" /B "%EDITOR%" "%PROJECT%" /Game/Lvl_ModularCarTest -server -nullrhi -ExecCmds="Net PktLag=90, car.SelfTest 9" -unattended -nopause -nosplash -noxgeshadercompile -abslog="%NETS%" -port=7777

echo Waiting 18s for server...
ping 127.0.0.1 -n 19 >nul

echo Starting client (log: %NETC%)...
"%EDITOR%" "%PROJECT%" 127.0.0.1:7777 -game -nullrhi -ExecCmds="Net PktLag=90, car.SelfTest 9" -unattended -nopause -nosplash -noxgeshadercompile -abslog="%NETC%"
set CLIENT_EXIT=%ERRORLEVEL%

echo Client exit code: %CLIENT_EXIT%
if exist "%NETC%" (
  findstr /C:"SELFTEST REMOTEDRIVE" "%NETC%"
) else (
